...
<head>
    <meta charset="UTF-8">
    <title>Service Purchase Success</title>
...
<body>
    <div class="payment-success-container">
        <div class="success-card">
            ...
            <h2>Service Purchase Confirmed!</h2>
            
            <p>Thank you for purchasing <strong class="for_this">{{ $service->name }}</strong></p>
            
            <div class="session-details">
                <p><strong>Service Type:</strong> {{ $service->is_hourly ? 'Hourly Service' : 'Fixed Price' }}</p>
                <p><strong>Amount Paid:</strong> R{{ number_format($payment->amount, 2) }}</p>
                <p><strong>Transaction ID:</strong> {{ $payment->transaction_id }}</p>
                @if($service->is_hourly)
                <p><strong>Hours Purchased:</strong> {{ $payment->metadata['hours'] ?? 1 }}</p>
                @endif
            </div>
